...
function calcDeudas(expenses, members) {
  ...
  expenses.forEach(function (exp) {
    var valid = (exp.divididoEntre || []).filter(function (m) {
  ...
  var deudores = members
    .filter(function (m) {
      return balance[m] < -0.01;
    })
    .map(function (m) {
      return { name: m, amount: balance[m] };
    });
  var acreedores = members
    .filter(function (m) {
      return balance[m] > 0.01;
    })
    .map(function (m) {
...
var SECTION_FIELDS = {
  ...
  alojamiento: [
    ...
    { label: "Tipo", key: "tipo", placeholder: "Hotel / Airbnb / Hostel" },
    {
      label: "Dirección",
      key: "direccion",
      placeholder: "Ej: Calle 5, Playa del Carmen",
    },
  ...
  excursiones: [
    ...
    { label: "Duración", key: "duracion", placeholder: "Ej: 8 horas" },
    {
      label: "Incluye",
      key: "incluye",
      placeholder: "Ej: Transporte, comida, guía",
    },
    {
  ...
  documentos: [
    {
      label: "Tipo *",
      key: "tipoDoc",
      placeholder: "Ej: Pasaporte, Seguro, Visa",
    },
    ...
    { label: "Vencimiento", key: "vencimiento", type: "date" },
    {
      label: "Notas",
      key: "notas",
      placeholder: "Ej: Cobertura médica $50.000",
    },
  ],
};
...
function getDateLine(section, item) {
  if (section === "vuelos")
    return item.fechaSalida ? "Salida: " + fmtDateTime(item.fechaSalida) : "";
  if (section === "alojamiento")
    return item.checkIn
      ? fmtDateTime(item.checkIn) +
          (item.checkOut ? " → " + fmtDateTime(item.checkOut) : "")
      : "";
  if (section === "auto")
    return item.fechaRecogida
      ? "Recogida: " + fmtDateTime(item.fechaRecogida)
      : "";
  if (section === "excursiones") return item.fecha ? fmtDate(item.fecha) : "";
  if (section === "entradas") return item.fecha ? fmtDate(item.fecha) : "";
  if (section === "documentos")
    return item.vencimiento ? "Vence: " + fmtDate(item.vencimiento) : "";
  return "";
}

function getSummary(section, item) {
  if (section === "vuelos")
    return [item.aerolinea, item.nroVuelo].filter(Boolean).join(" · ");
  if (section === "alojamiento")
    return [item.tipo, item.nroReserva ? "Res: " + item.nroReserva : ""]
      .filter(Boolean)
      .join(" · ");
  if (section === "auto") return item.lugarRecogida || "";
  if (section === "excursiones")
    return [item.horaEncuentro, item.puntoEncuentro]
      .filter(Boolean)
      .join(" · ");
  if (section === "entradas")
    return item.cantidad ? item.cantidad + " entradas" : "";
...
function Empty({ icon, text }) {
  return React.createElement(
    ...
    React.createElement(
      "p",
      { style: { margin: 0, fontSize: 14, color: "#64748b" } },
      text
    )
  );
}
...
function AttachSec({ attachments, onChange, color }) {
  ...
  function handleFoto(e) {
    ...
    r.onload = function () {
      var next = Object.assign({}, attachments, {
        foto: r.result,
        fotoNombre: f.name,
      });
      onChange(next);
  ...
  return React.createElement(
        ...
        return React.createElement(
          "button",
          {
            ...
            style: {
              ...
              borderRadius: 10,
              background:
                tabVal === t.id ? color + "20" : "rgba(255,255,255,0.04)",
              border:
                tabVal === t.id
                  ? "1px solid " + color + "55"
                  : "1px solid rgba(255,255,255,0.08)",
              color: tabVal === t.id ? color : "#64748b",
              fontSize: 11,
              fontWeight: 600,
              cursor: "pointer",
            },
    ...
    tabVal === "link" &&
      React.createElement("input", {
        ...
        onChange: function (e) {
          onChange(Object.assign({}, attachments, { link: e.target.value }));
        },
        style: inputSt,
      }),
    tabVal === "foto" &&
        ...
        React.createElement(
          "label",
          {
            style: {
              ...
              border:
                "2px dashed " +
                (attachments.foto ? color : "rgba(255,255,255,0.12)"),
              borderRadius: 12,
              padding: "18px",
              cursor: "pointer",
              marginBottom: 8,
          ...
          React.createElement(
            "button",
            {
              onClick: function () {
                downloadFile(
                  attachments.foto,
                  attachments.fotoNombre || "foto.jpg"
                );
              },
              style: {
                width: "100%",
        ...
        React.createElement(
          "label",
          {
            style: {
              ...
              border:
                "2px dashed " +
                (attachments.archivo ? color : "rgba(255,255,255,0.12)"),
              borderRadius: 12,
              padding: "22px",
              cursor: "pointer",
              marginBottom: 8,
          ...
          React.createElement(
            ...
            {
              style: {
                color: attachments.archivo ? color : "#64748b",
                fontSize: 13,
                margin: "8px 0 0",
              },
            },
            attachments.archivo
              ? attachments.archivoNombre || "Archivo subido"
              : "Subir PDF u otro archivo"
          ),
          React.createElement("input", {
            type: "file",
          ...
          React.createElement(
            "button",
            {
              onClick: function () {
                downloadFile(
                  attachments.archivo,
                  attachments.archivoNombre || "archivo"
                );
              },
              style: {
                width: "100%",
...
function ItemForm({ initial, section, onSave, onClose }) {
  ...
  fields.forEach(function (f) {
    initVals[f.key] = initial ? initial[f.key] || "" : "";
  });
  ...
  function save() {
    ...
    var item = Object.assign({}, initial || {}, values, {
      estado: estado,
        ...
        return React.createElement(
          "button",
          {
            ...
            style: {
              ...
              cursor: "pointer",
              background:
                estado === st ? e.color + "22" : "rgba(255,255,255,0.04)",
              border:
                estado === st
                  ? "1px solid " + e.color + "66"
                  : "1px solid rgba(255,255,255,0.08)",
              color: estado === st ? e.color : "#64748b",
              fontSize: 11,
              fontWeight: 600,
            },
          },
...
function ItemModal({ item, section, color, onClose, onSave, onDelete }) {
  ...
  if (mode === "delete")
    return React.createElement(
      ...
      React.createElement(
        "p",
        { style: { color: "#94a3b8", fontSize: 14, margin: "0 0 24px" } },
        "¿Eliminar ",
        React.createElement(
          "strong",
          { style: { color: "#f1f5f9" } },
          getTitle(section, item)
        ),
        "?"
      ),
      React.createElement(
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "h3",
          { style: { color: "#f8fafc", margin: 0, fontSize: 17 } },
          getTitle(section, item)
        ),
        React.createElement(
          "span",
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            att.link.replace(/^https?:\/\//, "").split("/")[0]
          )
        ),
        React.createElement(
          "span",
    ...
    att.foto &&
      React.createElement(
        ...
        React.createElement(
          ...
          {
            onClick: function () {
              downloadFile(att.foto, att.fotoNombre || "foto.jpg");
            },
            style: {
              width: "100%",
              padding: "9px",
    ...
    att.archivo &&
      React.createElement(
        ...
        React.createElement(
          ...
          {
            onClick: function () {
              downloadFile(att.archivo, att.archivoNombre || "archivo");
            },
            style: {
              width: "100%",
              padding: "9px",
...
function ExpenseForm({ initial, members, currency, onSave, onClose }) {
  ...
  var fech = fechSt[0];
  var setFech = fechSt[1];
  var pagSt = useState(
    initial ? initial.pagadoPor || members[0] || "" : members[0] || ""
  );
  var pag = pagSt[0];
  var setPag = pagSt[1];
  var entrSt = useState(
    initial ? initial.divididoEntre || members.slice() : members.slice()
  );
  var entr = entrSt[0];
  var setEntr = entrSt[1];
  ...
  return React.createElement(
    ...
    React.createElement("input", {
      ...
      value: fech,
      onChange: function (e) {
        setFech(e.target.value);
      },
      style: Object.assign({}, inputSt, { colorScheme: "dark" }),
    }),
...
function ExpenseModal({ exp, members, currency, onClose, onSave, onDelete }) {
  ...
  return React.createElement(
    ...
    React.createElement(
      "h3",
      { style: { color: "#f8fafc", margin: "0 0 4px", fontSize: 17 } },
      exp.descripcion
    ),
    React.createElement(
      "p",
      { style: { color: "#64748b", fontSize: 13, margin: "0 0 16px" } },
      exp.fecha ? exp.fecha + " · " : "",
      "Pagó ",
      React.createElement(
        "span",
        {
          style: { color: gc(members.indexOf(exp.pagadoPor)), fontWeight: 600 },
        },
        exp.pagadoPor
      )
    ),
    React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "p",
          { style: { color: "#64748b", fontSize: 11, margin: 0 } },
          "Total"
        ),
        React.createElement(
          "p",
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          currency + (exp.total / (exp.divididoEntre || [1]).length).toFixed(2)
        )
      )
    ),
    React.createElement(
...
function GastosView({ expenses, onUpdate, members, currency }) {
  var showSt = useState(false);
  var showAdd = showSt[0];
  var setShowAdd = showSt[1];
  ...
  function saveExp(exp) {
    var ex = expenses.find(function (e) {
      return e.id === exp.id;
    });
    onUpdate(
      "expenses",
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      [
        {
          label: "Total grupo",
          val: currency + total.toLocaleString(),
          color: "#f1f5f9",
        },
        {
          label: "Por persona",
          val:
            currency +
            (members.length ? (total / members.length).toFixed(0) : 0),
          color: "#4ade80",
        },
      ].map(function (c) {
        return React.createElement(
      ...
      members.map(function (m, i) {
        ...
        return React.createElement(
          ...
          React.createElement(
            ...
            {
              style: {
                color: b >= 0 ? "#4ade80" : "#f87171",
                fontWeight: 700,
                fontSize: 14,
                margin: 0,
              },
            },
            (b >= 0 ? "+" : "") + currency + Math.abs(b).toFixed(0)
          )
        );
      })
    ),
    ...
    React.createElement(
      ...
      React.createElement(
        ...
        {
          style: {
            background: tx.length ? "#f87171" : "#4ade80",
            color: "#fff",
            fontSize: 10,
            fontWeight: 700,
            borderRadius: 99,
    ...
    tx.map(function (t, i) {
      return React.createElement(
        ...
        React.createElement(Avt, {
          name: t.de,
          color: gc(members.indexOf(t.de)),
        }),
        React.createElement(
          "div",
          { style: { flex: 1 } },
    ...
    expenses.length === 0 &&
      React.createElement(Empty, {
        icon: "💰",
        text: "Sin gastos. Tocá + para agregar.",
      }),
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              (exp.fecha ? exp.fecha + " · " : "") + "Pagó ",
              React.createElement(
                "span",
                {
                  style: {
                    color: gc(members.indexOf(exp.pagadoPor)),
                    fontWeight: 600,
                  },
                },
                exp.pagadoPor
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            (exp.divididoEntre || [])
              ...
              .map(function (m) {
                return React.createElement(Avt, {
                  key: m,
                  name: m,
                  color: gc(members.indexOf(m)),
                  size: 22,
                });
              })
          ),
    ...
    showAdd &&
      React.createElement(
        ...
        React.createElement(ExpenseForm, {
          ...
          currency: currency,
          onSave: function (exp) {
            onUpdate("expenses", expenses.concat([exp]));
          },
          onClose: function () {
            setShowAdd(false);
          },
...
function AlertasView({ alertas, onUpdate }) {
  ...
  var editItem = editSt[0];
  var setEditItem = editSt[1];
  var titSt = useState("");
  var tit = titSt[0];
  var setTit = titSt[1];
  var fecSt = useState("");
  ...
  function add() {
    ...
    onUpdate(
      "alertas",
      alertas.concat([
        { id: Date.now(), titulo: tit, fecha: fec, tipo: tip, activa: true },
      ])
    );
    setTit("");
    setFec("");
  ...
  function saveEdit(upd) {
    onUpdate(
      "alertas",
      alertas.map(function (a) {
        return a.id === upd.id ? upd : a;
      })
    );
  }

  var proximas = alertas
    .filter(function (a) {
      return a.activa && a.fecha && new Date(a.fecha) >= hoy;
    })
    .sort(function (a, b) {
      return new Date(a.fecha) - new Date(b.fecha);
    });
  var pasadas = alertas.filter(function (a) {
    return !a.activa || (a.fecha && new Date(a.fecha) < hoy);
  });
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        "h2",
        {
          style: { color: "#f8fafc", fontSize: 18, fontWeight: 800, margin: 0 },
        },
        "🔔 Alertas"
      ),
      React.createElement(
    ...
    alertas.length === 0 &&
      React.createElement(Empty, {
        icon: "🔔",
        text: "Agregá recordatorios de fechas importantes.",
      }),
    proximas.length > 0 &&
      React.createElement(
        "div",
        ...
        proximas.map(function (a) {
          var dias = Math.ceil(
            (new Date(a.fecha) - hoy) / (1000 * 60 * 60 * 24)
          );
          return React.createElement(
            "div",
            {
            ...
            React.createElement(
              ...
              a.fecha &&
                React.createElement(
                  ...
                  React.createElement(
                    "span",
                    { style: { color: dias <= 3 ? "#f87171" : "#facc15" } },
            ...
            return React.createElement(
              "button",
              {
                ...
                style: {
                  ...
                  borderRadius: 10,
                  background:
                    tip === t ? "#f472b622" : "rgba(255,255,255,0.05)",
                  border:
                    tip === t
                      ? "1px solid #f472b655"
                      : "1px solid rgba(255,255,255,0.08)",
    ...
    editItem &&
      React.createElement(
          ...
          tipos.map(function (t) {
            return React.createElement(
              "button",
              {
                key: t,
                onClick: function () {
                  setEditItem(Object.assign({}, editItem, { tipo: t }));
                },
                style: {
                  width: 40,
                  height: 40,
        ...
        React.createElement("input", {
          ...
          onChange: function (e) {
            setEditItem(
              Object.assign({}, editItem, { titulo: e.target.value })
            );
          },
          style: inputSt,
        }),
        ...
        React.createElement("input", {
          ...
          onChange: function (e) {
            setEditItem(Object.assign({}, editItem, { fecha: e.target.value }));
          },
          style: Object.assign({}, inputSt, { colorScheme: "dark" }),
        }),
        React.createElement(
...
function ChecklistView({ checklist, members, onUpdate }) {
  ...
  var setCat = catSt[1];
  var cats = ["🎒", "👔", "💊", "📄", "🔌", "🎿", "🏖️", "💻"];
  var pendientes = checklist.filter(function (i) {
    return !i.completado;
  });
  var completados = checklist.filter(function (i) {
    return i.completado;
  });
  var prog = checklist.length
    ? Math.round((completados.length / checklist.length) * 100)
    : 0;
  ...
  function toggle(id) {
    onUpdate(
      "checklist",
      checklist.map(function (i) {
        return i.id === id
          ? Object.assign({}, i, { completado: !i.completado })
          : i;
      })
    );
  }
  ...
  function renderItem(item, done) {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "p",
          { style: { color: "#64748b", fontSize: 11, margin: "2px 0 0" } },
          item.asignado
        )
      ),
      React.createElement(
  ...
  return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            "span",
            { style: { color: "#64748b", fontSize: 12 } },
            completados.length + " de " + checklist.length + " listo"
          ),
          React.createElement(
            "span",
            { style: { color: "#34d399", fontWeight: 700, fontSize: 12 } },
            prog + "%"
          )
        ),
        React.createElement(
    ...
    checklist.length === 0 &&
      React.createElement(Empty, {
        icon: "✅",
        text: "Sin items. Agregá cosas para no olvidar.",
      }),
    pendientes.length > 0 &&
      React.createElement(
        "div",
    ...
    completados.length > 0 &&
      React.createElement(
        ...
        React.createElement(
          ...
          "Listo (" + completados.length + ")"
        ),
        completados.map(function (i) {
    ...
    showAdd &&
      React.createElement(
        ...
        React.createElement(
          ...
          ["Todos"].concat(members).map(function (m, i) {
            return React.createElement(
              "button",
              {
                key: m,
...
// ── Clima View ──
function ClimaView() {
  var ciudadSt = useState("");
  var ciudad = ciudadSt[0];
  var setCiudad = ciudadSt[1];
  var climaSt = useState(null);
  var clima = climaSt[0];
  var setClima = climaSt[1];
  ...
  var setErr = errSt[1];
  var sugSt = useState([]);
  var sugerencias = sugSt[0];
  var setSugerencias = sugSt[1];
  var showSugSt = useState(false);
  var showSug = showSugSt[0];
  var setShowSug = showSugSt[1];
  ...
  function buscar(ciudadParam) {
    ...
    fetch("https://wttr.in/" + encodeURIComponent(q.trim()) + "?format=j1")
      ...
      .catch(function () {
        setErr("Ciudad no encontrada. Probá con otra.");
        setLoading(false);
      });
  }
  ...
  return React.createElement(
    ...
    !clima &&
      ...
      React.createElement(
        ...
        React.createElement(
          "div",
          { style: { textAlign: "center", padding: "30px 20px 20px" } },
          React.createElement(
            "div",
          ...
          React.createElement(
            "p",
            { style: { color: "#64748b", fontSize: 14, margin: 0 } },
            "Escribí la ciudad y buscá el clima"
          )
        ),
        React.createElement(
          "p",
    ...
    clima &&
      React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                "p",
                { style: { color: "#94a3b8", fontSize: 14, margin: 0 } },
          ...
          React.createElement(
            ...
            [
              {
                label: "Sensación",
                val: clima.current_condition[0].FeelsLikeC + "°",
              },
              {
                label: "Humedad",
                val: clima.current_condition[0].humidity + "%",
              },
              {
                label: "Viento",
                val: clima.current_condition[0].windspeedKmph + " km/h",
              },
            ].map(function (d) {
              return React.createElement(
                "div",
        ...
        (clima.weather || []).slice(0, 3).map(function (day, i) {
          ...
          var nombre = fecha.toLocaleDateString("es-ES", {
            weekday: "long",
            day: "numeric",
            month: "short",
          });
          var code =
            day.hourly && day.hourly[4] ? day.hourly[4].weatherCode : "113";
          var desc =
            day.hourly && day.hourly[4]
          ...
          return React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                "p",
                {
                  style: { color: "#64748b", fontSize: 13, margin: "2px 0 0" },
                },
                day.mintempC + "°"
              )
            )
...
function ConversorView({ currency }) {
  var montSt = useState("");
  var mont = montSt[0];
  var setMont = montSt[1];
  var desdeSt = useState("USD");
  var desde = desdeSt[0];
  var setDesde = desdeSt[1];
  var hastaSt = useState("EUR");
  var hasta = hastaSt[0];
  var setHasta = hastaSt[1];
  var ratesSt = useState({});
  var rates = ratesSt[0];
  var setRates = ratesSt[1];
  var loadSt = useState(false);
  var loading = loadSt[0];
  var setLoading = loadSt[1];
  var errSt = useState("");
  ...
  function getMon(code) {
    return (
      monedas.find(function (m) {
        return m.code === code;
      }) || { code: code, flag: "💱", name: code }
    );
  }
    ...
    fetch("https://api.frankfurter.app/latest?from=" + base)
      ...
      .then(function (d) {
        if (d.rates) {
          ...
          setErr("No se pudieron cargar las tasas");
        }
        setLoading(false);
      })
      .catch(function () {
  ...
  var resultado =
    mont && rates[hasta] ? (parseFloat(mont) * rates[hasta]).toFixed(2) : "";
    ...
    React.createElement(
      ...
      tasa &&
        React.createElement(
          ...
          React.createElement(
            "span",
            { style: { color: "#3d5166", fontSize: 11 } },
      ...
      React.createElement(
        ...
        [1, 5, 10, 20, 50, 100, 500].map(function (v) {
          return React.createElement(
            ...
            React.createElement(
              "span",
              { style: { color: "#38bdf8", fontWeight: 700, fontSize: 14 } },
...
function GrupoView({ members, expenses, currency }) {
  var res = calcDeudas(expenses, members);
  var balance = res.balance;
  var totalGastado = expenses.reduce(function (s, e) {
    return s + e.total;
  }, 0);
  ...
  return React.createElement(
    ...
    members.length === 0 &&
      React.createElement(Empty, {
        icon: "👥",
        text: "Sin integrantes. Agregá desde Configuración.",
      }),
    members.map(function (m, i) {
      var b = balance[m] || 0;
      var gm = expenses.filter(function (e) {
        return e.pagadoPor === m;
      });
      var tm = gm.reduce(function (s, e) {
        return s + e.total;
      }, 0);
      return React.createElement(
        "div",
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            {
              style: {
                background: b >= 0 ? "#4ade8022" : "#f8717122",
                border: b >= 0 ? "1px solid #4ade8044" : "1px solid #f8717144",
                borderRadius: 12,
                padding: "6px 12px",
                textAlign: "center",
              },
            ...
            React.createElement(
              "p",
              { style: { color: "#64748b", fontSize: 10, margin: "2px 0 0" } },
              b >= 0 ? "le deben" : "debe"
            )
          )
        ),
        React.createElement(
          ...
          { style: { display: "flex", gap: 10 } },
          [
            { label: "Pagó", val: currency + tm.toFixed(0) },
            { label: "Gastos", val: String(gm.length) },
            {
              label: "% total",
              val:
                (totalGastado ? Math.round((tm / totalGastado) * 100) : 0) +
                "%",
            },
          ].map(function (c) {
            return React.createElement(
...
function ConfigView({ data, onUpdate, onReset, onLeave }) {
  ...
  var lnSt = useState(data.tripName);
  var ln = lnSt[0];
  var setLn = lnSt[1];
  var ldSt = useState(data.dates);
  var ld = ldSt[0];
  var setLd = ldSt[1];
  var lmSt = useState(data.members.slice());
  var lm = lmSt[0];
  var setLm = lmSt[1];
  var lcSt = useState(data.currency);
  var lc = lcSt[0];
  var setLc = lcSt[1];
  var nmSt = useState("");
  var nm = nmSt[0];
  var setNm = nmSt[1];
  var confSt = useState(false);
  var conf = confSt[0];
  var setConf = confSt[1];
  var copSt = useState(false);
  var cop = copSt[0];
  var setCop = copSt[1];
  ...
  function copyCode() {
    try {
      navigator.clipboard.writeText(data.tripCode);
    } catch (e) {}
    setCop(true);
    setTimeout(function () {
      setCop(false);
  ...
  function share() {
    ...
    try {
      if (navigator.share) navigator.share({ title: "Vamo", text: msg });
      else navigator.clipboard.writeText(msg);
    } catch (e) {}
  }

  var totalItems = Object.values(data.items).reduce(function (s, arr) {
    return s + arr.length;
  }, 0);
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        "p",
        { style: { color: "#64748b", fontSize: 12, margin: "0 0 14px" } },
    ...
    React.createElement(
      ...
      [
        { label: "Nombre", val: data.tripName },
        { label: "Fechas", val: data.dates },
        { label: "Moneda", val: data.currency },
        { label: "Integrantes", val: String(data.members.length) },
        { label: "Reservas", val: String(totalItems) },
        { label: "Gastos", val: String(data.expenses.length) },
      ].map(function (row) {
        return React.createElement(
          "div",
          {
          ...
          return React.createElement(
            "button",
            {
              ...
              style: {
                padding: "8px 14px",
                borderRadius: 10,
                background: lc === c ? "#38bdf822" : "rgba(255,255,255,0.05)",
                border:
                  lc === c
                    ? "1px solid #38bdf855"
                    : "1px solid rgba(255,255,255,0.08)",
                color: lc === c ? "#38bdf8" : "#64748b",
                fontWeight: 700,
                fontSize: 14,
                cursor: "pointer",
              },
      ...
      React.createElement(
        ...
        lm.map(function (m, i) {
          return React.createElement(
            ...
            React.createElement(
              "span",
              { style: { color: gc(i), fontSize: 12, fontWeight: 700 } },
              m
            ),
            lm.length > 1 &&
              React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        "p",
        { style: { color: "#64748b", fontSize: 12, margin: "0 0 12px" } },
        "Volvés al inicio. El viaje sigue existiendo."
      ),
      React.createElement(
        "button",
        {
    ...
    React.createElement(
      ...
      React.createElement(
        "p",
        { style: { color: "#64748b", fontSize: 12, margin: "0 0 12px" } },
        "Borra todas las reservas, gastos y alertas."
      ),
      !conf
        ? React.createElement(
            "button",
...
function HomeScreen({ onEnter, onCreate }) {
  ...
  var codeSt = useState("");
  var code = codeSt[0];
  var setCode = codeSt[1];
  var errSt = useState("");
  var err = errSt[0];
  var setErr = errSt[1];
  var savedSt = useState([]);
  var saved = savedSt[0];
  ...
  function handleEnter() {
    ...
    if (c.length < 4) {
      setErr("Ingresá un código válido");
      return;
    }
    setErr("");
    onEnter(c);
  ...
  return React.createElement(
    ...
    React.createElement("div", {
      style: {
        ...
        borderRadius: "50%",
        background:
          "radial-gradient(ellipse,rgba(129,140,248,0.10) 0%,transparent 65%)",
        pointerEvents: "none",
      },
    }),
    ...
    React.createElement(
      "div",
      { style: { position: "relative", zIndex: 1, padding: "72px 28px 48px" } },
      // Logo
      React.createElement(
        "div",
        ...
        React.createElement(
          "p",
          { style: { color: "#475569", fontSize: 15, margin: 0 } },
          "Organizá tu viaje con tu grupo"
        )
      ),
      ...
      React.createElement(
        ...
        "Cada viaje tiene un código único · Compartilo con tu grupo"
      )
    )
  );
}
...
export default function App() {
  var darkSt = useState(function () {
    try {
      return localStorage.getItem("vamo_theme") !== "light";
    } catch (e) {
      return true;
    }
  });
  var dark = darkSt[0];
  var setDark = darkSt[1];

  function toggleTheme() {
    setDark(function (d) {
      var nd = !d;
      try {
        localStorage.setItem("vamo_theme", nd ? "dark" : "light");
      } catch (e) {}
      return nd;
    });
  }

  var screenSt = useState("home");
  var screen = screenSt[0];
  var setScreen = screenSt[1];
  var codeSt = useState("");
  var tripCode = codeSt[0];
  var setTripCode = codeSt[1];
  var dataSt = useState(null);
  var data = dataSt[0];
  var setData = dataSt[1];
  var navSt = useState("viaje");
  var nav = navSt[0];
  var setNav = navSt[1];
  var secSt = useState("alojamiento");
  var activeSection = secSt[0];
  var setActiveSection = secSt[1];
  var selSt = useState(null);
  var selectedItem = selSt[0];
  var setSelectedItem = selSt[1];
  var addSt = useState(false);
  var showAdd = addSt[0];
  var setShowAdd = addSt[1];
  var syncSt = useState("conectando");
  var syncStatus = syncSt[0];
  var setSyncStatus = syncSt[1];
  var nfSt = useState(false);
  var notFound = nfSt[0];
  var setNotFound = nfSt[1];
  ...
  function saveToLocal(code) {
    try {
      ...
      if (!l.includes(code)) {
        l.unshift(code);
        localStorage.setItem("my_trips", JSON.stringify(l.slice(0, 10)));
      }
    } catch (e) {}
  }
  ...
  function saveFB(nd) {
    setDoc(doc(db, "viajes", nd.tripCode), nd).catch(function (e) {
      console.error(e);
    });
  }
  ...
  var currentSection = SECTIONS.find(function (s) {
    return s.id === activeSection;
  });
  var currentItems = data ? data.items[activeSection] || [] : [];
  var totalGrupo = data
    ? data.expenses.reduce(function (s, e) {
        return s + e.total;
      }, 0)
    : 0;
  var pendingAlertas = data
    ? (data.alertas || []).filter(function (a) {
        return a.activa && a.fecha && new Date(a.fecha) >= new Date();
      }).length
    : 0;

  function saveItem(item) {
    var ex = currentItems.find(function (i) {
      return i.id === item.id;
    });
    updateItems(
      activeSection,
  ...
  function deleteItem(id) {
    updateItems(
      activeSection,
      currentItems.filter(function (i) {
        return i.id !== id;
      })
    );
  }
  ...
  if (screen === "home")
    return React.createElement(
      ThemeCtx.Provider,
      { value: { dark: dark, toggle: toggleTheme } },
        ...
        notFound
          ? React.createElement(
              ...
              React.createElement(
                "h2",
                {
                  style: { color: T.text, margin: "0 0 8px", fontWeight: 800 },
                },
                "Viaje no encontrado"
              ),
              React.createElement(
          ...
          : React.createElement(
              ...
              React.createElement(
                "p",
                { style: { color: T.text2, fontSize: 14 } },
  ...
  return React.createElement(
    ...
    React.createElement(
      ...
      React.createElement(
        "div",
        {
          style: { padding: "52px 24px 16px", position: "relative", zIndex: 1 },
        },
        React.createElement(
          "div",
          {
          ...
          React.createElement(
            ...
            React.createElement(
              "p",
              { style: { color: T.text2, fontSize: 13, margin: "4px 0 0" } },
              data.dates +
                " · " +
                data.members.length +
                " " +
                (data.members.length === 1 ? "viajero" : "viajeros")
            )
          ),
          totalGrupo > 0 &&
            React.createElement(
              ...
              React.createElement(
                ...
                data.currency + totalGrupo.toLocaleString()
              )
            )
        ),
        React.createElement(
          ...
          data.members.slice(0, 6).map(function (m, i) {
            return React.createElement(Avt, {
              key: m,
              name: m,
              color: gc(i),
      ...
      React.createElement(
        ...
        nav === "viaje" &&
          React.createElement(
                  ...
                  React.createElement(
                    "span",
                    {
                      style: {
                        fontSize: 12,
                        fontWeight: 700,
                        color: active ? sec.color : T.text3,
                        whiteSpace: "nowrap",
                      },
                    },
                    sec.label
            ...
            activeSection === "gastos"
              ? React.createElement(GastosView, {
              ...
              : React.createElement(
                    ...
                    React.createElement(
                      "button",
                      {
                        ...
                        style: {
                          ...
                          alignItems: "center",
                          justifyContent: "center",
                          boxShadow: "0 4px 16px " + secColor + "25",
                        },
                      },
                      "+"
                    )
                  ...
                  currentItems.length === 0 &&
                    React.createElement(Empty, {
                      icon: currentSection ? currentSection.icon : "📋",
                      text: "Sin reservas. Toca el + para agregar.",
                    }),
                      ...
                      React.createElement(
                        ...
                        React.createElement(
                          ...
                          React.createElement(
                            ...
                            getTitle(activeSection, item)
                          ),
                          getDateLine(activeSection, item) &&
                            React.createElement(
                              "p",
                              {
                                style: {
        ...
        nav === "grupo" &&
          React.createElement(GrupoView, {
            members: data.members,
            expenses: data.expenses,
            currency: data.currency,
          }),
        nav === "alertas" &&
          React.createElement(AlertasView, {
        ...
        nav === "conversor" &&
          React.createElement(ConversorView, { currency: data.currency }),
        nav === "config" &&
